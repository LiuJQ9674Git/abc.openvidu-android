...
public class CustomWebSocket extends AsyncTask<SessionActivity, Void, Void>
    ...
    public void onTextMessage(WebSocket websocket, String text) throws Exception {
        ...
        if (json.has(JsonConstants.RESULT)) {//Key中包括RESULT
    ...
    private void handleServerResponse(JSONObject json) throws
        ...
        } else if (rpcId == this.ID_PUBLISHVIDEO.get()) {//发布视频
            // Response to publishVideo
            SessionDescription sessionDescription =
                    new SessionDescription(SessionDescription.Type.ANSWER, result.getString("sdpAnswer"));
            //  应答信息在PC中有createAnswer方法，但是没有调用
            this.session.getLocalParticipant().getPeerConnection().
                    setRemoteDescription(new CustomSdpObserver("localSetRemoteDesc"), sessionDescription);

        } else if (this.IDS_RECEIVEVIDEO.containsKey(rpcId)) {//接收
            // Response to receiveVideoFrom
            SessionDescription sessionDescription =
                    new SessionDescription(SessionDescription.Type.ANSWER, result.getString("sdpAnswer"));
            //  远端应答
            //  本端的远端应答，应答信息在PC中有createAnswer方法，但是没有调用
            session.getRemoteParticipant(IDS_RECEIVEVIDEO.remove(rpcId)).getPeerConnection().
                    setRemoteDescription(new CustomSdpObserver("remoteSetRemoteDesc"), sessionDescription);

        } else if (this.IDS_ONICECANDIDATE.contains(rpcId)) {//候选者在线
    ...
    private void joinRoom() {
        ...
        joinRoomParams.put(JsonConstants.METADATA, "{\"clientData\": \"" +
                this.session.getLocalParticipant().getParticipantName() + "\"}");
    ...
    public void publishVideo(SessionDescription sessionDescription) {
        ...
        this.ID_PUBLISHVIDEO.set(this.sendJson(JsonConstants.PUBLISHVIDEO_METHOD,
                publishVideoParams));
    }
    ...
    public void receiveVideoFrom(SessionDescription sessionDescription,
        ...
        this.IDS_RECEIVEVIDEO.put(this.sendJson(JsonConstants.RECEIVEVIDEO_METHOD, receiveVideoFromParams),
    ...
    private void participantPublishedEvent(JSONObject params) throws
        ...
        final String streamId = params.getJSONArray("streams").
    ...
    private void participantLeftEvent(JSONObject params) throws JSONException {
        final RemoteParticipant remoteParticipant =
                this.session.removeRemoteParticipant(params.getString("connectionId"));
    ...
    private RemoteParticipant newRemoteParticipantAux(JSONObject participantJson) throws JSONException {
        ...
        //实例化RemoteParticipant
        final RemoteParticipant remoteParticipant = new RemoteParticipant(connectionId,
                participantName, this.session);
        //设置视图的创建远程候选者
    ...
    private void subscribeAux(RemoteParticipant remoteParticipant, String streamId) {
        ...
        remoteParticipant.getPeerConnection().createOffer(new CustomSdpObserver("remote offer sdp") {
            ...
            public void onCreateSuccess(SessionDescription sessionDescription) {
                ...
                //远端提交交互请求之后，回调中设置本端的SDP的描述信息
                remoteParticipant.getPeerConnection().setLocalDescription(new
                        CustomSdpObserver("remoteSetLocalDesc"), sessionDescription);
    ...
    public void onDisconnected(WebSocket websocket, WebSocketFrame
        ...
        Log.e(TAG, "Disconnected " + serverCloseFrame.getCloseReason() + " " +
                clientCloseFrame.getCloseReason() + " " + closedByServer);
    }
